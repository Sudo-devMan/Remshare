...
function ReceivedFiles() {
  ...
  const [oneD, setOneD] = useState<{ b: boolean; url: string }>({
    b: false,
    url: "",
  });
  const [copied, setCopied] = useState("Click to copy to clipboard");
  const isFiles = shareData.files.length < 0;
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      LOCAL_STORAGE_FILES,
      JSON.stringify(organizeFiles(formatFiles(shareData.files))),
    );
  }, []);
  ...
  const triggerDownload = async (fileUrl: string) => {
    ...
    try {
      ...
      console.error(
        "Failed to force download, falling back to open window.",
        error,
      );
    ...
    <div className="w-full p-1">
      ...
      <div className="sm:grid sm:grid-cols-2 px-5 items-start gap-4">
        <div className="border border-2 rounded-lg w-full sm:w-2/3 p-3 sm:m-0 mb-2">
          ...
          <br />
          <p className="caveat text-xl sm:text-2xl">
            <span className="text-blue-700 font-bold">Receive Link: </span>
            <button
              className="receive-sm"
              onClick={() => copy(shareData.receiveUrl)}
            >
              {copied}
            </button>
          </p>
          ...
          <p className="caveat text-xl sm:text-2xl">
            ...
            {shareData.note ? shareData.note : "NO NOTE PROVIDED"}
          </p>
          {downloading ? (
            <button
              disabled
              className="text-2xl w-full p-3 transform duration-300 text-3xl bg-green-600 border-2 rounded-4xl mt-3 text-white caveat border border-black"
            >
            ...
            <button
              ...
              className="w-full p-3 cursor-pointer hover:bg-green-500 transform duration-300 text-3xl bg-green-600 border-2 rounded-4xl active:bg-green-700 mt-3 text-white caveat border border-black"
            >
          ...
          <Link
            to={"/"}
            className="caveat bg-white p-2 rounded-3xl text-xl border border-black"
          >
            Back to home
          </Link>
        </div>
        ...
        <div className="border border-2 rounded-lg p-3">
          <div>
            {files && files.images.length !== 0 && (
              <div>
                <h1 className={filesHead.split("my-3")[0]}>
                  Pictures: ({files.images.length}){" "}
                </h1>
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {files.images.map((k, i) => {
                    return (
                      k.type === FileType.IMAGE && (
                        <div key={i} className="m-0">
                          <a target="_blank" href={k.url}>
                            <img
                              ...
                              className="border border-blue-600 transform transition-transform duration-300 hover:scale-105 hover:shadow-blue-300 shadow shadow-blue-900"
                              alt="My girl's selfie"
                            />
                          </a>
                        </div>
                      )
            ...
            {files && files.videos.length !== 0 && (
              <div>
                <h1 className={filesHead}>Videos: ({files.videos.length})</h1>
                <div className="sm:grid sm:grid-cols-2 gap-2">
                  {files.videos.map((v, i) => {
                    return (
                      v.type === FileType.VIDEO && (
                        <div
                          key={i}
                          className="flex flex-col bg-blue-100 rounded-xl p-2 items-center justify-between border"
                        >
                          ...
                          {oneD.b && oneD.url === v.url ? (
                            <button
                              disabled
                              className="w-full border mt-2 py-1 rounded-lg bg-blue-200 border-blue-500 text-sm font-medium transition-colors"
                            >
                              Downloading...{" "}
                            ...
                            <button
                              onClick={() => {
                                triggerDownload(v.url);
                                setOneD({ b: true, url: v.url });
                              }}
                              className="w-full border mt-2 py-1 rounded-lg hover:bg-blue-300 active:bg-blue-500 bg-blue-200 border-blue-500 text-sm font-medium transition-colors"
                            >
                ...
                {files.audios.map((a, i) => {
                  return (
                    a.type === FileType.AUDIO && (
                      <div
                        key={i}
                        className="mb-1 flex flex-col bg-blue-100 rounded-xl p-3 items-center justify-between border w-full"
                      >
                        ...
                        {oneD.b && oneD.url === a.url ? (
                          ...
                          <button
                            onClick={() => {
                              triggerDownload(a.url);
                              setOneD({ b: true, url: a.url });
                            }}
                            className="w-full border mt-2 py-1 rounded-lg hover:bg-blue-300 active:bg-blue-500 bg-blue-200 border-blue-500 text-sm font-medium transition-colors"
                          >
                            Download <FontAwesomeIcon icon={faDownload} />
            ...
            {files && files.other.length > 0 && (
              <div>
                <h1 className={filesHead}>
                  Other file types and documents: ({files.other.length})
                </h1>
                <div className="sm:grid sm:grid-cols-2 sm:gap-2 p-3 text-xl">
                  {files.other.map((o, i) => {
                    return (
                      o.type === FileType.OTHER && (
                        <div
                          key={i}
                          className="p-2 border rounded-lg bg-blue-100 mb-1 text-wrap"
                        >
                          ...
                          <div className="grid grid-cols-2 w-full">
                            ...
                            {oneD.b && oneD.url === o.url ? (
                              <button
                                disabled
                                className="border p-1 rounded-lg bg-blue-200 border-blue-500 text-sm font-medium transition-colors"
                              >
                                Downloading...{" "}
                                <FontAwesomeIcon icon={faDownload} />
                              </button>
                              ...
                              <button
                                onClick={() => {
                                  triggerDownload(o.url);
                                  setOneD({ b: true, url: o.url });
                                }}
                                className="border p-1 rounded-lg hover:bg-blue-300 active:bg-blue-500 bg-blue-200 border-blue-500 text-sm font-medium transition-colors"
                              >
                                Download <FontAwesomeIcon icon={faDownload} />
                              </button>
                            )}
